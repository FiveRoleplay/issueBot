...
client.on('message', async msg => {
  ...
  } else if (msg.content.includes("!sumsun?")) {
    const channel = client.channels.cache.find(
      (test) => test.name === msg.channel.name
    );
  ...
  // }
    ...
    const channel = client.channels.cache.find(
      (elmt) => elmt.name === msg.channel.name
    );
    ...
    const isDev = msg.member.roles.cache.some(
      ({name}) => name.toLowerCase() === "dev"
    );
    const isModo = msg.member.roles.cache.some(
      ({name}) => name.toLowerCase() === "modération"
    );
